                      ...
                      itemBuilder: (context, index) {
                        return InkWell(
                          ...
                          child:Container(
                            ...
                            border: _attrMap[attribute.id].contains(attribute.values[index]) ? Border.all(color: Colors.black, width: 1.5, ) : Border.all(color: Colors.grey[300],),
                            ),
                            ...
                            child: ConstrainedBox(
                              ...
                              child: Container(
                              color: attribute.attributeControlType == AttributeControlType.ColorSquares? parseColor(attribute.values[index].colorSquaresRgb):null,
                              child:attribute.attributeControlType == AttributeControlType.ColorSquares?null:
                        Center(child:Text(attribute.values[index].name +
                                  (attribute.values[index].priceAdjustment?.isNotEmpty == true ? ' (${attribute.values[index].priceAdjustment})' : ''), style: TextStyle(),
  ...
  void _settingModalBottomSheet(context, CustomAttribute attribute) {
    ...
    attribute.values.forEach((element) {
      ...
      if (attribute.attributeControlType == AttributeControlType.ColorSquares)
        leading = Container(width: 40, height: 40, color: parseColor(element.colorSquaresRgb));
      else if (attribute.attributeControlType == AttributeControlType.ImageSquare)
        leading = CpImage(url: element.imageSquaresPictureModel.imageUrl,width: 50,height: 50);

      listings.add(ListTile(
        title: Text(element.name +
              (element.priceAdjustment?.isNotEmpty == true ? ' (${element.priceAdjustment})' : ''), style: TextStyle(),
        ),
  ...
  String checkRequiredAttributes(List<CustomAttribute> attributes) {
    ...
    attributes.forEach((attribute) {
      if (attribute.isRequired && _attrMap[attribute.id].isEmpty)
        errorMsg = '$errorMsg${attribute.name} ${GlobalService().getString(Const.IS_REQUIRED)}\n';
    });
