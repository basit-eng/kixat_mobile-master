...
import 'package:kixat/views/customWidget/order_total_table.dart';
...
class OrderDetailsScreen extends StatefulWidget {
  ...
  _OrderDetailsScreenState createState() =>
      _OrderDetailsScreenState(orderId: this.orderId);
}
  ...
  Widget getRootWidget(OrderDetailsData confirmModel) {
    var orderInfoCard = Card(
      child: ListTile(
        ...
        title: Text(
            '${_globalService.getString(Const.ORDER_NUMBER)} ${confirmModel.customOrderNumber}'),
        subtitle: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
                '${_globalService.getString(Const.ORDER_STATUS)}: ${confirmModel.orderStatus}'),
            Text(
                '${_globalService.getString(Const.ORDER_DATE)}: ${getFormattedDate(confirmModel.createdOn)}'),
            Text(
                '${_globalService.getString(Const.ORDER_TOTAL)}: ${confirmModel.orderTotal}'),
          ],
        ),
      ),
    );
    ...
    var billingAdrsCard = Card(
      child: ListTile(
        ...
        subtitle: Text(getFormattedAddress(confirmModel.billingAddress)),
      ),
    );
    ...
    var shippingAdrsCard = Card(
      child: ListTile(
        ...
        subtitle: Text(getFormattedAddress(confirmModel.shippingAddress)),
      ),
    );
    ...
    var storeAdrsCard = Card(
      child: ListTile(
        ...
        subtitle: Text(getFormattedAddress(confirmModel.pickupAddress)),
      ),
    );
          ...
          children: [
            ...
            if (confirmModel.canRePostProcessPayment)
              OutlinedButton(
                ...
                child:
                    Text(_globalService.getString(Const.ORDER_RETRY_PAYMENT)),
              ),
          ],
        ),
      ),
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return ListTile(
            ...
            subtitle: Text(
                '${_globalService.getString(Const.PRICE)}: ${confirmModel.items[index].unitPrice}'),
            onTap: () =>
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          children: [
            orderInfoCard,
            if (confirmModel.billingAddress != null) billingAdrsCard,
            if (confirmModel.isShippable && !confirmModel.pickupInStore)
              shippingAdrsCard,
            if (confirmModel.pickupInStore) storeAdrsCard,
            shippingMethodCard,
            paymetnMethodCard,
            if (confirmModel.checkoutAttributeInfo?.isNotEmpty == true)
              selectedAttributes,
            if (confirmModel.shipments?.isNotEmpty == true) shipmentCard,
            if (confirmModel.orderNotes?.isNotEmpty == true) notesCard,
            cartItems,
            SizedBox(height: 10),
            OrderTotalTable(
                orderTotals: OrderTotals(
                    ...
                    paymentMethodAdditionalFee:
                        confirmModel.paymentMethodAdditionalFee,
                    redeemedRewardPoints:
                        confirmModel.redeemedRewardPoints ?? 0,
                    redeemedRewardPointsAmount:
                        confirmModel.redeemedRewardPointsAmount,
